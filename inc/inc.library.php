...
function recehan($nom)
{
	$dataa = $nom % 100000;
	$u100k = ($nom - $dataa) / 100000;

	$datab = $dataa % 50000;
	$u50k = ($dataa - $datab) / 50000;

	$datac = $datab % 20000;
	$u20k = ($datab - $datac) / 20000;

	$datad = $datac % 10000;
	$u10k = ($datac - $datad) / 10000;

	$datae = $datad % 5000;
	$u5k = ($datad - $datae) / 5000;

	$dataf = $datae % 2000;
	$u2k = ($datae - $dataf) / 2000;

	$datag = $dataf % 1000;
	$u1k = ($dataf - $datag) / 1000;

	$datah = $datag % 500;
	$u500 = ($datag - $datah) / 500;

	$datai = $datah % 100;
	$u100 = ($datah - $datai) / 100;

	return array($u100k, $u50k, $u20k, $u10k, $u5k, $u2k, $u1k, $u500, $u100);
}

function digitNoBukti($noBukti)
{
	$digit = sprintf("%05d", $noBukti);
