...
if (mysqli_num_rows($sqlKegiatan) > 0) {
    ...
    while ($viewPanitia = mysqli_fetch_array($sqlPanitia)) {
        ...
        $mpdf->WriteHTML('</table>');
        $mpdf->WriteHTML('<div>
            <p>Honorarium ' . $namaJabatan . ' dan Perjalanan Dinas dari ' . $viewPanitia['kabkota_unit_kerja'] . ' - ' . str_replace('Kabupaten', 'Kab.', ucwords($viewKeg['nama_kab'])) . ' PP<br>' . $viewKeg['nama_keg_kuitansi'] . '<br>Tanggal ' . Indonesia2Tgl($viewPanitia['tgl_mulai']) . ' s.d ' . Indonesia2Tgl($viewPanitia['tgl_selesai']) . '</p>
            </div>');
        ...
        $mpdf->WriteHTML('<tr>
            ...
            <td width="15%" style="text-align:right;">' . format_angka($transportPanitia) . ',-</td>
            <td></td>
            <td></td>
            </tr>');
        $mpdf->WriteHTML('<tr>
            <td width="27%">Uang Harian (' . $jmlHariPanitia . ' Hari)</td>
            <td width="2%">:</td>
            <td width="2%" style="">Rp. </td>
            <td width="15%" style="text-align:right;">' . format_angka($uangHarianPanitia) . ',-</td>
            <td></td>
            <td></td>
            </tr>');
        $mpdf->WriteHTML('<tr>
            ...
            <td width="2%">:</td>
            <td width="2%" style="">Rp. </td>
            <td width="15%" style="text-align:right;">' . format_angka($uangHonorPanitiaPotongan) . ',-</td>
            <td></td>
            <td></td>
            </tr>');
        $mpdf->WriteHTML('<tr>
