        ...
        <div class="float-end">
            <div class="btn-group">
                ...
                <ul class="dropdown-menu dropdown-menu-end dropdown-menu-lg-end">
                    <li>
                        <button type="button" class="dropdown-item d-flex align-items-center waves-effect" title="Tambah Panitia dari unsur Internal" data-bs-toggle="modal" data-bs-target="#addCommitteeInternal" data-id="<?= encrypt($id_keg) ?>"><i class="mdi mdi-account-multiple-plus text-success"></i>&nbsp;Panitia Internal</button>
                    </li>
                    <li>
                        <button type="button" class="dropdown-item d-flex align-items-center waves-effect" title="Tambah Panitia dari unsur Eksternal" data-bs-toggle="modal" data-bs-target="#addCommitteeExternal" data-id="<?= encrypt($id_keg) ?>"><i class="mdi mdi-account-multiple-plus text-success"></i>&nbsp;Panitia Eksternal</button>
                    </li>
                </ul>
            </div>
        </div>
                  ...
                  order by tb_jabatan_kegiatan.kd_jabatan asc");
                    ...
                    ?>
                        <tr>
                            ...
                            <td class="text-center"><?= $totalTransport == '' ? '0' : format_angka($totalTransport) ?></td>
                            ...
                            <td class="text-center"><?= format_angka($totalTransport + $uangHarianPanitia + $uangHonorPanitiaPotongan) ?></td>
                            <td class="text-center text-nowrap">
                                <?php if ($viewPanitia['jenis_panitia'] == 'eksternal') { ?>
                                    <button class="btn btn-icon btn-sm btn-info waves-effect me-1 mb-1" title="Edit Data Panitia Eksternal" type="button" data-bs-toggle="modal" data-bs-target="#editCommitteeExternal" data-id="<?= encrypt($viewPanitia['id_panitia_keg_hotel']) ?>" data-token="<?= encrypt($id_keg) ?>"><span class="tf-icons mdi mdi-file-edit-outline"></span></button>
                                <?php } ?>
                                <button class="btn btn-icon btn-sm btn-primary waves-effect me-1 mb-1" title="Edit Kehadiran Panitia" type="button" data-bs-toggle="modal" data-bs-target="#attendanceCommittee" data-id="<?= encrypt($viewPanitia['id_panitia_keg_hotel']) ?>" data-token="<?= encrypt($id_keg) ?>"><span class="tf-icons mdi mdi-forum-outline"></span></button>
                                <button class="btn btn-icon btn-sm btn-danger waves-effect me-1 mb-1" title="Hapus Panitia" type="button" data-bs-toggle="modal" data-bs-target="#delCommittee" data-id="<?= encrypt($viewPanitia['id_panitia_keg_hotel']) ?>" data-token="<?= encrypt($id_keg) ?>"><span class="tf-icons mdi mdi-delete"></span></button>
                            </td>
                        </tr>
                    <?php } ?>
                </tbody>
