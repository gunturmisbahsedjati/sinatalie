        ...
        <div class="float-end">
            <div class="btn-group">
                ...
                <ul class="dropdown-menu dropdown-menu-end dropdown-menu-lg-end">
                    <li>
                        <button type="button" class="dropdown-item d-flex align-items-center waves-effect" title="Tambah Narasumber dari unsur Internal" data-bs-toggle="modal" data-bs-target="#addInformantInternal" data-id="<?= encrypt($id_keg) ?>"><i class="mdi mdi-account-multiple-plus text-success"></i>&nbsp;Narasumber Internal</button>
                    </li>
                    <li>
                        <button type="button" class="dropdown-item d-flex align-items-center waves-effect" title="Tambah Narasumber dari unsur Eksternal" data-bs-toggle="modal" data-bs-target="#addInformantExternal" data-id="<?= encrypt($id_keg) ?>"><i class="mdi mdi-account-multiple-plus text-success"></i>&nbsp;Narasumber Eksternal</button>
                    </li>
                </ul>
            </div>
        </div>
                ...
                <tbody>
                    ...
                    while ($viewNarsum = mysqli_fetch_array($sqlNarsum)) {
                        ...
                        if ($viewNarsum['jenis_narsum'] == 'internal') {
                            $uangHarianNarsum = $viewPerjadin['uh_narsum_i'] == '' ? '0' : $viewPerjadin['uh_narsum_i'] * $jmlHari;
                            $uangHonorNarsum = 0;
                            $hotel = 0;
                            $statusKemdikbud = '';
                        ...
                        <tr>
                            ...
                            <td class="text-center text-nowrap">
                                <?php if ($viewNarsum['jenis_narsum'] == 'eksternal') { ?>
                                    <button class="btn btn-icon btn-sm btn-info waves-effect me-1 mb-1" title="Edit Data Narasumber Eksternal" type="button" data-bs-toggle="modal" data-bs-target="#editInformantExternal" data-id="<?= encrypt($viewNarsum['id_narsum_keg_hotel']) ?>" data-token="<?= encrypt($id_keg) ?>"><span class="tf-icons mdi mdi-file-edit-outline"></span></button>
                                    <button class="btn btn-icon btn-sm btn-primary waves-effect me-1 mb-1" title="Edit SPJ Narasumber" type="button" data-bs-toggle="modal" data-bs-target="#inputTransportInformant" data-id="<?= encrypt($viewNarsum['id_narsum_keg_hotel']) ?>" data-token="<?= encrypt($id_keg) ?>"><span class="tf-icons mdi mdi-cash"></span></button>
                                <?php } ?>
                                <button class="btn btn-icon btn-sm btn-danger waves-effect me-1 mb-1" title="Hapus Narasumber" type="button" data-bs-toggle="modal" data-bs-target="#delInformant" data-id="<?= encrypt($viewNarsum['id_narsum_keg_hotel']) ?>" data-token="<?= encrypt($id_keg) ?>"><span class="tf-icons mdi mdi-delete"></span></button>
                            </td>
                        </tr>
                    <?php } ?>
                </tbody>
