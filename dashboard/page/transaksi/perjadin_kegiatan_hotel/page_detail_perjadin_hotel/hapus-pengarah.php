...
if (isset($_POST['id']) && isset($_POST['token'])) {
    ...
    $sqlPeserta = mysqli_query($myConnection, "select tb_pengarah_keg_hotel.*, tb_kabkota.name as nama_kab
    from tb_pengarah_keg_hotel
    left join tb_kabkota on tb_kabkota.id = tb_pengarah_keg_hotel.id_kabkota_unit_kerja
    where tb_pengarah_keg_hotel.id_keg = '$id_keg' and tb_pengarah_keg_hotel.id_pengarah_keg_hotel = '$id_pengarah_keg_hotel'");
...
?>
        <form action="setDocumentHotel" method="post" role="form" enctype="multipart/form-data" autocomplete="off">
            ...
            <div class="modal-body">
                <h4><i class="bx bx-data"></i> Hapus Data Pengarah</h4>
                ...
                <div class="pt-2">
                    <div class="form-check">
                        <div class="custom-control custom-checkbox">
                            <input type="checkbox" class="form-check-input" id="del_pengarah" name="cek">
                            <label class="form-check-label" for="del_pengarah">Saya yakin menghapus Peserta.</label>
                        </div>
                    </div>
                </div>
            </div>
            <div class="modal-footer">
                ...
                <button type="submit" name="delDirector" class="btn btn-info" id="delPengarah" disabled>Hapus</button>
                <button type="button" class="btn btn-danger" data-bs-dismiss="modal" aria-label="Close">Batal</button>
            </div>
        </form>
        <script type="text/javascript">
            $('#del_pengarah').click(function() {
                if ($(this).is(':checked')) {

                    $('#delPengarah').removeAttr('disabled');

                } else {
                    $('#delPengarah').attr('disabled', true);
                }
            });
        </script>
<?php
