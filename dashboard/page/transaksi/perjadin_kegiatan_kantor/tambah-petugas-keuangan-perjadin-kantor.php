...
?>
    <div class="modal-header">
        <h4><i class="bx bx-folder"></i> Daftar Petugas Keuangan SPJ Kantor</h4>
            ...
            <div class="mb-5">
                <div class="input-group">
                    <select class="form-control border border-select-picker1" title="Pilih Petugas Keuangan...." data-live-search="true" data-size="5" id="petugas_keu" name="petugas_keu">
                        ...
                        <?php
                        $sqlKeu = mysqli_query($myConnection, "select tb_panitia_keg_kantor.*, tb_pegawai.nama_peg as nama_peg, tb_jabatan_kegiatan.nama_jabatan
                  ...
                  order by tb_jabatan_kegiatan.kd_jabatan asc");
                        while ($viewKeu = mysqli_fetch_array($sqlKeu)) {
                            echo '<option value="' . $viewKeu['id_panitia_keg_kantor'] . '">' . $viewKeu['nama'] . '</option>';
                        }
                        ?>
                    </select>
                    <button class="btn btn-outline-primary waves-effect" type="submit" name="addFinancialOfficeClerk">Tambah</button>
                </div>
        ...
        <div class="table-responsive text-nowrap">
            <table id="tabel_keu" class="table table table-bordered table-hover" width="100%" cellspacing="0">
                ...
                <tbody>
                    <?php
                    $no = 1;
                    $sqlPetugasKeu = mysqli_query($myConnection, "select tb_petugas_keuangan_keg_hotel.*, tb_panitia_keg_kantor.nama
                    from tb_petugas_keuangan_keg_hotel
                    left join tb_panitia_keg_kantor on tb_panitia_keg_kantor.id_panitia_keg_kantor = tb_petugas_keuangan_keg_hotel.id_panitia_keg_hotel
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                <form action="setDocumentOffice" method="post" role="form" enctype="multipart/form-data" autocomplete="off">
                                    ...
                                    <input type="hidden" name="_type" value="<?= $page ?>">
                                    <button class="btn btn-icon btn-outline-danger waves-effect" title="Hapus Petugas Keuangan" type="submit" name="delFinancialOfficeClerk"><span class="tf-icons mdi mdi-delete"></span></button>
                                </form>
                            </td>
                        </tr>
                    <?php } ?>
