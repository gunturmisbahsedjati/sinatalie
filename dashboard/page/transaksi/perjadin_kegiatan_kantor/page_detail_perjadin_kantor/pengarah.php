<ul class="nav nav-pills mb-3 nav-fill" role="tablist">
    ...
    <li class="nav-item ms-2 me-2 mb-2">
        <a href="checkDocumentOffice?_token=<?= encrypt($id_keg) ?>&_type=pengarah" class="nav-link active border border-primary">
        ...
        <div class="float-end">
            <div class="btn-group">
                ...
                <ul class="dropdown-menu dropdown-menu-end dropdown-menu-lg-end">
                    <li>
                        <button type="button" class="dropdown-item d-flex align-items-center waves-effect" title="Tambah Pengarah dari unsur Internal" data-bs-toggle="modal" data-bs-target="#addDirectorInternalOffice" data-id="<?= encrypt($id_keg) ?>"><i class="mdi mdi-account-multiple-plus text-success"></i>&nbsp;Pengarah Internal</button>
                    </li>
                    <li>
                        <button type="button" class="dropdown-item d-flex align-items-center waves-effect" title="Tambah Pengarah dari unsur Eksternal" data-bs-toggle="modal" data-bs-target="#addDirectorExternalOffice" data-id="<?= encrypt($id_keg) ?>"><i class="mdi mdi-account-multiple-plus text-success"></i>&nbsp;Pengarah Eksternal</button>
                    </li>
                </ul>
            </div>
        </div>
                        ...
                        <tr>
                            ...
                            <td class="text-center text-nowrap">
                                <?php if ($viewPengarah['jenis_pengarah'] == 'eksternal') { ?>
                                    <button class="btn btn-icon btn-sm btn-info waves-effect me-1 mb-1" title="Edit Data Narasumber Eksternal" type="button" data-bs-toggle="modal" data-bs-target="#editDirectorExternalOffice" data-id="<?= encrypt($viewPengarah['id_pengarah_keg_kantor']) ?>" data-token="<?= encrypt($id_keg) ?>"><span class="tf-icons mdi mdi-file-edit-outline"></span></button>
                                    <button class="btn btn-icon btn-sm btn-primary waves-effect me-1 mb-1" title="Edit SPJ Narasumber" type="button" data-bs-toggle="modal" data-bs-target="#inputTransportDirectorOffice" data-id="<?= encrypt($viewPengarah['id_pengarah_keg_kantor']) ?>" data-token="<?= encrypt($id_keg) ?>"><span class="tf-icons mdi mdi-cash"></span></button>
                                <?php } ?>
                                <button class="btn btn-icon btn-sm btn-danger waves-effect me-1 mb-1" title="Hapus Pengarah" type="button" data-bs-toggle="modal" data-bs-target="#delDirectorOffice" data-id="<?= encrypt($viewPengarah['id_pengarah_keg_kantor']) ?>" data-token="<?= encrypt($id_keg) ?>"><span class="tf-icons mdi mdi-delete"></span></button>
                            </td>
                        </tr>
                    <?php } ?>
                </tbody>
