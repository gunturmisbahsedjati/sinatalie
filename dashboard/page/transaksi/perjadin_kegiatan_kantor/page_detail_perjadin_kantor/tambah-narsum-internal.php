...
?>
        <form action="setDocumentOffice" method="post" role="form" enctype="multipart/form-data" autocomplete="off">
            <div class="modal-header">
                <h4><i class="bx bx-data"></i> Tambah Narasumber Internal</h4>
            </div>
            <div class="modal-body">
                <div class="mb-3">
                    <label class="form-label">Daftar Pegawai Internal</label>
                    <select class="form-control border border-select-picker1" title="Pilih Pegawai Internal...." data-live-search="true" data-size="10" id="narsum_internal" name="narsum_internal" required>
                        <?php
                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input class="form-control border border-secondary" id="tgl_mulai_narsum" placeholder="dd-mm-yyyy" name="tgl_mulai" value="<?= $viewKeg['tgl_mulai'] == '' || $viewKeg['tgl_mulai'] == '0000-00-00' ? '' : tanggal($viewKeg['tgl_mulai']); ?>">
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="mb-3">
                            <label class="form-label">Tanggal Akhir</label>
                            <input class="form-control border border-secondary" id="tgl_selesai_narsum" placeholder="dd-mm-yyyy" name="tgl_selesai" value="<?= $viewKeg['tgl_selesai'] == '' || $viewKeg['tgl_selesai'] == '0000-00-00' ? '' : tanggal($viewKeg['tgl_selesai']); ?>">
                        </div>
                    </div>
                </div>
            </div>
            <div class="modal-footer">
                ...
                <button type="submit" name="addInternalInformantOffice" class="btn btn-info">Simpan</button>
                <button type="button" class="btn btn-danger" data-bs-dismiss="modal" aria-label="Close">Batal</button>
            </div>
        </form>
        <script type="text/javascript">
            $('#narsum_internal').selectpicker();
            $('#tgl_mulai_narsum').datepicker({
                uiLibrary: 'bootstrap5',
                format: 'dd-mm-yyyy'
            });
            $('#tgl_selesai_narsum').datepicker({
                uiLibrary: 'bootstrap5',
                format: 'dd-mm-yyyy'
            });
        </script>
