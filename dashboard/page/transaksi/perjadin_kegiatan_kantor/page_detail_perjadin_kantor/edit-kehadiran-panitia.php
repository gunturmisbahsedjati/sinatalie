...
if (isset($_POST['id']) && isset($_POST['token'])) {
    ...
    $id_panitia_keg_kantor = decrypt($_POST['id']);
    $sqlPanitia = mysqli_query($myConnection, "select tb_panitia_keg_kantor.*, tb_kabkota.name as nama_kab
    from tb_panitia_keg_kantor
    left join tb_kabkota on tb_kabkota.id = tb_panitia_keg_kantor.id_kabkota_unit_kerja
    where tb_panitia_keg_kantor.id_keg = '$id_keg' and tb_panitia_keg_kantor.id_panitia_keg_kantor = '$id_panitia_keg_kantor'");
...
?>
        <form action="setDocumentOffice" method="post" role="form" enctype="multipart/form-data" autocomplete="off">
            ...
            <div class="modal-footer">
                ...
                <input type="hidden" value="<?= encrypt($id_panitia_keg_kantor) ?>" name="_id">
                <button type="submit" name="editAttendanceCommitteeOffice" class="btn btn-info" id="kehadiranPanitia" disabled>Simpan</button>
